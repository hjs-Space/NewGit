...
{   //创建一个上下方向分割的JSplitPane对象
	...
	private JButton[] jbArray={//设置JButton按钮的文本
			...
			new JButton("修改学生信息"),new JButton("查找学生信息"),new JButton("确认修改")
	};
	...
	public Student()
	{
		...
		jp.setLayout(null);//设置面板的上部分为空布局管理器
		jsp.setDividerLocation(130);//设置jp中分割条的初始位置
		...
		for(int i=0;i<7;i++){
			...
			{//对界面上的第一行标签和文本框大小位置进行设置
				jlArray[i].setBounds(25+i*220,10,100,20);
				jtxtArray[i].setBounds(105+i*200,10,120,20);
				jtxtArray[i].addActionListener(this);
			}
			else if(i>2&&i<6)
			{//对第二行标签和文本框大小位置进行设置
				jlArray[i].setBounds(25+(i-3)*220,50,100,20);
				jtxtArray[i].setBounds(105+(i-3)*200,50,120,20);
				jtxtArray[i].addActionListener(this);
			}
			else
			{//对最下面的显示标签进行设置
				jlArray[i].setBounds(650,10,100,20);
			}
		}
		this.add(jsp);
		jp.add(jcp);
		jsp.setBottomComponent(jspn);//设置下部子窗格
		jcp.setBounds(730,10,100,20);
		for(int i=0;i<5;i++)
		{//将JButton添加进jp
			jp.add(jbArray[i]);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(e.getSource()==jbArray[2])
		{
			...
			int row = this.jt.getSelectedRow();//声明所选行号
			...
			if(row>=0){
				//选择了一行
				for(int i=0;i<6;i++){jtxtArray[i].setText(jt.getValueAt(row,i).toString());}
			}

			/*updateDialog up=new updateDialog("修改", jt, row);
	...
	/*else{//若Record表中有该学号，则输出提示对话框
				JOptionPane.showMessageDialog(this,	"该学生已经删除！",
							        "消息",JOptionPane.INFORMATION_MESSAGE);
				//更新table并显示
	        	dtm.setDataVector(data,head);
	    		jt.setModel(dtm);
	...
	public void updateStudent(){
		...
		str[6]=jcp.getSelectedItem().toString();
